/* ═══════════════════════════════════════
   CREATORMIND AI — SCRIPT.JS
   ═══════════════════════════════════════ */

'use strict';

// ═══════════════════════════════════════
// ── Users who choose "Use System API" will use this key ──
const SYSTEM_API_KEY = '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'; // 🔑 Replace with your Groq API key
// ═══════════════════════════════════════

// ── State ──────────────────────────────────
const state = {
  apiMode: localStorage.getItem('cm_api_mode') || 'system', // 'system' | 'own'
...
function initApiSetup() {
  // Restore saved mode
  setApiMode(state.apiMode, false);

  // Restore saved own key
  ...
  $('saveApiKey')?.addEventListener('click', () => {
    ...
    showToast('✓ Your API key saved!');
  });
}

// Toggle between system and own key modes
function setApiMode(mode, save = true) {
  state.apiMode = mode;
  if (save) localStorage.setItem('cm_api_mode', mode);

  const btnSystem = $('modeSystem');
  const btnOwn = $('modeOwn');
  const panelSystem = $('panelSystem');
  const panelOwn = $('panelOwn');
...
// Returns the active API key based on selected mode
function getActiveApiKey() {
  if (state.apiMode === 'system') {
...
async function handleGenerate() {
  ...
  if (!activeKey || activeKey === 'YOUR_GROQ_API_KEY_HERE') {
    if (state.apiMode === 'system') {
      showToast('⚠ System API key not configured. Switch to "Use My Own Key".');
    } else {
...
function buildPrompt({ topic, type, tone, platform, length, context }) {
  ...
  let systemPrompt = `You are CreatorMind AI, an elite content creation specialist. 
...
function renderAnalytics(content) {
  ...
  // Calculate pseudo-scores based on content analysis
...
// ═══════════════ TESTIMONIALS ═══════════════
const TESTIMONIALS = [
  { name: 'Verified Human', role: 'YouTuber · 500K subs', rating: 5, text: 'CreatorMind AI completely changed my content workflow. I generate a week of video scripts in under 30 minutes. The hooks are insanely good.' },
  ...
  { name: 'Marketing Ninja', role: 'TikTok Creator · 1M subs', rating: 5, text: 'The viral hooks are unreal. I copy one, shoot a quick video, and it blows up. CreatorMind AI basically does the hard creative thinking for me.' },
...
// ═══════════════ FAQ ═══════════════
const FAQS = [
  { q: 'How does AI content generation work?', a: 'CreatorMind AI connects to advanced language models (Groq/LLaMA) via API. You provide a topic, type, and tone — the AI generates platform-optimized content in seconds. No complex setup needed.' },
  ...
  { q: 'Which AI model does the platform use?', a: 'CreatorMind AI uses LLaMA 3.3 70B via Groq\'s ultra-fast inference API. This gives you high-quality, GPT-4 level content at blazing speeds — often under 2 seconds per generation.' },
  ...
  { q: 'Is my API key secure?', a: 'Your API key is stored only in your browser\'s localStorage and is never sent to our servers. All API calls are made directly from your browser to Groq\'s servers. Your data stays private.' },
];
...
// Re-run observer after initial render
...
function initKeyboardShortcuts() {
  document.addEventListener('keydown', e => {
    ...
    // Ctrl+K to focus topic input
    if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
      e.preventDefault();
      $('topicInput')?.focus();
      $('topicInput')?.select();
    }
    // Ctrl+C shortcut for copy (when output panel focused)
